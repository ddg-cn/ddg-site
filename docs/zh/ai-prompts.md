...
> "你是一位拥有 15 年经验的供应链合规审核员，熟悉 BSCI、SMETA、SLCP、SA8000、RBA、ISO 14001 及 LkSG 法规要求。"
...
> 反面例子："生成一份合规问卷。"
> 正确示范："生成一份符合 LkSG §5 尽职调查要求和 CSDDD Article 10 风险评估要求的供应商自评问卷。"
...
* 编号列表（适合问卷题目）
* Word 风格的分节文档
* Excel 可导入的 CSV 格式（需注明列标题）

### 原则四：要求评分标准

对于问卷和检查清单，要求 AI 同时生成评分标准，可以极大降低后续审核打分的主观性。

> "为每道题目提供三档评分标准：满分（完全合规）、部分分（部分合规/有证据但不完整）、零分（不合规/无证据）。"
...
任务：生成一份供应商自评问卷（SAQ），用于对我们的一级直接供应商进行年度合规自评。
...
追问 2 — 制作双语版本：
将以上问卷的所有中文题目翻译为英文，输出为两列表格：
左列：中文原文；右列：英文译文。
保持题号对应，翻译风格专业（参照 “[名词解释](glossary)”）。

追问 3 — 特定主题深化：
将"劳工权利"的部分从 10 道题扩展为 25 道题，
...
审核类型：\[BSCI / SMETA 4-pillar / SLCP / SA8000 / ISO 14001 / RBA VAP / 定制]
审核日期：\[X 周后]
...
任务：根据以下审核发现描述，生成一份标准格式的审核发现项报告（Audit Finding Report）。
...
要求：
- 按 (BSCI CAP / SMETA CAP / RBA CAP 或其他项目 CAP ) 格式输出每条发现项，包含：
...
```
\[企业信息] 起草一份负责任采购政策，涵盖供应商筛选标准、尽职调查流程、持续监控机制和改进路径，参照 CSDDD（和或LkSG 等） 要求，约 800–1200 字，中英双语。
```

**反贿赂反腐败政策（ABAC Policy）**

```
\[企业信息] 起草一份符合《联合国反腐败公约》（ 和/或UK Bribery Act 等）要求的反贿赂反腐败政策，包含：禁止行为定义、礼品与招待限额标准、举报渠道、违规处理程序，约 600–1000 字。
```

**环境政策（Environmental Policy）**

```
\[企业信息] 起草一份企业环境政策，涵盖气候承诺（碳中和路径）、水资源管理、废弃物减量、生物多样性，参照 ISO 14001 框架，约 500–800 字，中英双语。
```

**申诉机制说明（Grievance Mechanism）**

```
\[企业信息] 起草一份面向供应商工人的申诉机制说明文件（工人版，语言简明易懂），说明如何举报、谁来处理、如何保密举报内容和举报者，且举报者不会遭受报复的承诺，约 400–600 字，中英双语。
```
...
5. 输出格式：
   ...
   - 风险项详细评分表（供填写用）
   - 风险等级汇总仪表盘说明
6. 在末尾说明该矩阵如何与  CSDDD "比例原则"（和/或LkSG §5 "风险分析"等）要求对应
```
...
```
以双语格式输出：每段中文后紧跟对应英文译文。
英文译文使用供应链合规领域的专业术语惯例（参照“[名词解释](glossary)”）。
```
...
```
重新审阅你生成的内容，将所有引用的法规条款编号、
罚款金额、截止日期和统计数据用 \[待核实] 标记。
对于你不确定的引用，说明"建议查阅 \[官方来源名称] 核实"。
...
```
对你刚才生成的 \[文件名称] 进行完成度自评：
1. 按百分制，给整份文件打一个完成度评分（满分 100 分）
2. 列出你认为该文件目前缺少的内容（按重要性排序）
3. 列出在达到"审核就绪"前，还需要补充哪些内容
4. 假如你是一位有经验的合规专员，将该初稿完善为可用文件，你需要花多少时间？
```

### 技巧八：使用特定AI技能（例如，在 Claude 中使用 Projects 功能保存背景块）
